...
export const getConditions = () => async dispatch => {
  try {
    ...
    const machines=await axiosService.get('/conditions');
    dispatch({ type: GET_CONDITIONS_SUCCESS, payload: machines });
  } catch (error) {
...
export const getConditionByID = machineId => async dispatch => {
  try {
...
export const deleteCondition= machine =>async dispatch=>{
  try{
    ...
    const deletedCondition=await axiosService.delete('/machine/'+machine.id);
    dispatch({type:DELETE_CONDITION_SUCCESS,payload:machine});
