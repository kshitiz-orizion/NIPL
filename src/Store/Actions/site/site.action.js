...
export const getSites = () => async dispatch => {
  try {
    ...
    const sites=await axiosService.get('/rbac/location/sites/')
...
export const getSiteByID = siteId => async dispatch => {
  try {
    var site={name:'',id:''};
    if(siteId!==null){
    site = await axiosService.get('/rbac/location/sites/'+siteId);
    }
...
export const createSite = machines => async dispatch => {
  try{
    ...
    const createdsite=await axiosService.post('/rbac/location/sites/',machines,{'Content-type':'multipart/form-data'});
...
export const editSite = machines => async dispatch => {
  try {
    ...
    const machineEdit=await axiosService.put('/rbac/location/sites/'+machines.id+'/',machines,{'Content-type':'multipart/form-data'});
...
export const deleteSite= machine =>async dispatch=>{
  try{
    ...
    await axiosService.delete('/rbac/location/sites/'+machine.id);
